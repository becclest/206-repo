...
class TestCard(unittest.TestCase):
    def test_create(self):
        testCard1 = Card()
        self.assertTrue(testCard1.suit, "Diamonds")

        testCard2 = Card (0,1)
        self.assertTrue(testCard2.suit, "Diamonds")
        self.assertTrue(testCard2.rank, "Ace")

        testCard3 = Card (suit=2,rank=13)
        self.assertTrue(testCard3.suit, "Hearts")
...
class TestDeck(unittest.TestCase):
    ...
    def test_deck_size(self):
        """deck should fail with input over 52"""
        testDeck1 = Deck()
        (str(testDeck1).split('\n'))
        deckCount = (str(testDeck1).count('\n'))
        self.assertIs(deckCount, 51)


    def test_pop_card (self, i=-1):
        testDeck2 = Deck()
        new_testDeck2 = self.pop_card(testDeck2)
        assertEqual(testDeck2, new_testDeck2)


    def test_replace_card(self, card):
        self.card.replace_card(0, 1)
        testDeck4 = Deck()
        if card not in testDeck4:
            self.cards.append(card)
            self.assertTrue(testDeck4)
